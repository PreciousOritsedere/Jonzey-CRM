...
import { loadColumns } from './configureEmail_campaignsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListEmail_campaigns from './ListEmail_campaigns';

const perPage = 10;

const TableSampleEmail_campaigns = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: email_campaignsNotify,
    refetch,
  } = useAppSelector((state) => state.email_campaigns);
  ...
  useEffect(() => {
    if (email_campaignsNotify.showNotification) {
      notify(
        email_campaignsNotify.typeNotification,
        email_campaignsNotify.textNotification,
      );
    }
  }, [email_campaignsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/email_campaigns/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/email_campaigns/email_campaigns-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {email_campaigns && Array.isArray(email_campaigns) && !showGrid && (
        <ListEmail_campaigns
...
export default TableSampleEmail_campaigns;
